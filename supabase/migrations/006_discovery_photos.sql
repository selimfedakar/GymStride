-- Add profile_photo_url to discovery RPC return types so BuddyCard can show real photos.
...
BEGIN
  ...
  GROUP BY p.id, p.username, p.full_name, p.date_of_birth, p.experience_level, p.location, p.profile_photo_url
...
BEGIN
  ...
  GROUP BY p.id, p.username, p.full_name, p.date_of_birth, rp.run_types,
           rp.avg_pace_seconds_per_km, p.location, p.profile_photo_url
  ORDER BY badge_overlap_count DESC, distance_km ASC;
END;
$$;
