-- GymStride — 017: Cast run_types to text[] for comparison — no operator issues
-- Instead of constructing ARRAY[v_run_type] (which causes type inference issues),
-- we cast rp.run_types to text[] and compare directly with TEXT parameter.
-- = ANY(text[]) is universally supported and requires no custom operators.
...
-- ── Step 2: Explicit drops for every known old signature ──────────────────────
...
BEGIN
  ...
  WHERE p.id <> p_current_user_id
    ...
    AND (
      COALESCE(TRIM(p_filter_run_type), '') = ''
      OR p_filter_run_type = ANY(rp.run_types::text[])
    )
    AND (
      v_loc IS NULL
