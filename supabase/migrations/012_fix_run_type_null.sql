-- GymStride — 012: Fix run_type NULL handling in discover_running_buddies
-- Supabase-js sends JSON null as the text type, causing:
--   "operator does not exist: run_type = text"
-- Fix: accept TEXT for the filter param and cast internally.
...
BEGIN
  ...
  -- Cast text → run_type safely; NULL stays NULL
  v_run_type := CASE
    WHEN p_filter_run_type IS NULL THEN NULL
