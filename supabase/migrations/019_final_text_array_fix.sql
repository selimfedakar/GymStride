-- GymStride — 019: Absolute final fix
-- WHERE  : cast column to text[] and use && (overlap) against text[] param
-- SELECT : cast column back to run_type[] so return type matches declaration
-- PARAM  : p_run_types TEXT[] (null = no filter, array = must overlap)

-- Drop every overload by pg_proc
DO $$
DECLARE r RECORD;
BEGIN
  FOR r IN
...
-- Belt-and-suspenders explicit drops for all known signatures
...
BEGIN
  ...
  WHERE p.id <> p_current_user_id
    ...
    AND (p_run_types IS NULL OR rp.run_types::text[] && p_run_types)
    AND (
      v_loc IS NULL
