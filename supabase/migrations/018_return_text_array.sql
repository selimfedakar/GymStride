-- GymStride — 018: Fix 42804 return type mismatch
-- running_preferences.run_types column is text[] in the actual DB (not run_type[]).
-- All previous migrations declared RETURNS TABLE with run_type[], causing mismatch.
-- Fix: return TEXT[] to match the actual column, filter with = ANY() on matching types.
