-- GymStride — 013: Nuclear drop of ALL discover_running_buddies overloads
-- Migration 012 may have left an old overload alive if the DROP signature
-- didn't match exactly. This script drops every version via pg_proc then
-- recreates the function with a TEXT parameter (immune to supabase-js null casting).
--
...
-- Step 2: Recreate — p_filter_run_type is TEXT so JSON null never causes a type clash
...
BEGIN
  ...
  -- Safe cast: TEXT → run_type; NULL stays NULL without operator error
  v_run_type := CASE
    WHEN p_filter_run_type IS NULL OR p_filter_run_type = '' THEN NULL
...
-- Step 3: Force PostgREST to reload its schema cache immediately
