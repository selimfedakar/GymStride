-- GymStride — 015: Fix text[] @> run_type[] operator error
-- Explicit ::public.run_type[] cast on the ARRAY literal prevents
-- PostgreSQL from inferring text[] type on the right-hand side.
...
-- Step 2: Recreate with explicit run_type[] cast
...
BEGIN
  ...
  WHERE p.id <> p_current_user_id
    ...
    AND (
      v_run_type IS NULL
      OR rp.run_types @> ARRAY[v_run_type]::public.run_type[]
    )
    AND (
      v_loc IS NULL
